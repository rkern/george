...
kernels::Kernel* parse_kernel_spec (const py::object& kernel_spec) {
  ...
  switch (kernel_type) {
    ...
    case 1: {
      ...
      if (metric_type == 0) {
        kernel = new kernels::GammaExp<metrics::IsotropicMetric> (
        ...
        kernel = new kernels::GammaExp<metrics::AxisAlignedMetric> (
        ...
        kernel = new kernels::GammaExp<metrics::GeneralMetric> (
